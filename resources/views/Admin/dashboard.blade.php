...
<div x-data="{deleting : false, updating: false, add: false}" class="overflow-x-hidden mt-12 flex flex-col justify-center items-center">
    ...
    <div class="mb-12 gap-96 flex justify-between items-center">
        ...
        <form action="" class="w-64 pb-3 pr-2 flex items-center border-b border-b-slate-300 text-slate-300 focus-within:border-b-slate-900 focus-within:text-slate-900 transition">
            <input id="search" value="{{ request()->search }}" class="bg-neutral-50 px-2 w-full outline-none leading-none placeholder-slate-400" type="search" name="search" placeholder="Rechercher un utilisateur">
            <button>
    ...
    <table class="text-left p-1 border-collapse shadow-md w-fit">
        ...
        @forelse($users as $user)
            <tr class="odd:bg-stone-100 even:bg-stone-200">
                ...
                @endif
                <td>
                    <button id="updateButton" value="{{$user->id }}" type="button" x-on:click="updating = !updating" class="w-full flex justify-center hover:scale-125 transition duration-200">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
                ...
                <td>
                    <button id="deleteButton" value="{{$user->id }}" type="button" x-on:click="deleting = !deleting" class="w-full flex justify-center hover:scale-125 transition duration-200">
