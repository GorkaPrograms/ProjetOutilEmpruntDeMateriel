<div x-show="deleting">
    <div class="bg-black opacity-30 h-screen w-screen absolute top-0 left-0 z-10" x-on:click="deleting = !deleting"></div>
    ...
    <div class="bg-white absolute top-1/2 left-1/2 z-20 transform -translate-x-1/2 -translate-y-1/2 min-w-[33.333333%] w-auto h-auto rounded-md">
        ...
        <form action="{{ $action }}" method="POST" class="flex flex-col h-full w-full">
            ...
            <div class="w-full flex justify-between px-24 mb-4">
                ...
                <button type="button" class="w-1/4 px-8 py-1 bg-blue-700 text-gray-50 rounded-lg font-bold {{--Partie hover--}} hover:bg-blue-600 transition duration-200" x-on:click="deleting = !deleting">Non</button>
            </div>
        </form>
    </div>
</div>
